...
class AppUtils {
  ...
  static final Map<String, Function> _methods = {};

  // Register a method with a unique key
  static void register(String key, Function method) {
    _methods[key] = method;
  }

  // Call a method by key
  static void call(String key) {
  ...
  static void showTopBarMessage(BuildContext context, String message, {Duration? duration, Color? backgroundColor, Widget? icon, bool? isWithAppBar}) {
    _flushbar = Flushbar(
      positionOffset: isWithAppBar == true ? kToolbarHeight + AppSize.topSafeAreaPadding : AppSize.topSafeAreaPadding,
      margin: const EdgeInsets.all(15),
      borderRadius: BorderRadius.circular(8),
      flushbarPosition: FlushbarPosition.TOP,
      message: message,
      duration: duration ?? Constant.defaultTopBarDuration,
      backgroundColor: backgroundColor ?? context.color.backgroundPopup,
      messageColor: context.colorScheme.inverseSurface,
      icon: icon,
  ...
  static Future<void> changeStatusBarColor(bool isDarkTheme, {bool isUpdateStatusBar = true}) async {
    try {
      Log.d('changeStatusBarColor > isDarkTheme: $isDarkTheme');
      // FlutterStatusbarcolor.setStatusBarColor(Colors.transparent);
      // FlutterStatusbarcolor.setNavigationBarColor(isDarkTheme ? Colors.black : Colors.white);
      //Determine if the status bar text color should be dark or light based on brightness
      if (isDarkTheme) {
        if (isUpdateStatusBar) FlutterStatusbarcolor.setStatusBarWhiteForeground(true); // White text
        FlutterStatusbarcolor.setNavigationBarWhiteForeground(true);
      } else {
        if (isUpdateStatusBar) FlutterStatusbarcolor.setStatusBarWhiteForeground(false); // Dark text
        FlutterStatusbarcolor.setNavigationBarWhiteForeground(false);
      }
    } on PlatformException catch (e) {
      Log.e('_changeStatusBarColor > $e');
    }
  }
  ...
  static Future<void> preCacheSvg(String path) async {
    final loader = SvgAssetLoader(path);
    await svg.cache.putIfAbsent(loader.cacheKey(null), () => loader.loadBytes(null));
  }

  static Size calculateTextSize({String? text, double? maxWidth, TextStyle? style, int? maxLines, double fontScale = 1.0}) {
    final scaledStyle = style?.copyWith(fontSize: style.fontSize != null ? style.fontSize! * fontScale : null);
  ...
  static String? getDataByLanguage(String? content, String? lang) {
    try {
      final data = jsonDecode(content ?? '');
      if (data is Map<String, dynamic>) {
        return data[lang] ?? (data.isNotEmpty ? (data[Constant.defaultLanguageCode] ?? data.entries.first.value) : '');
      }
      return content;
    } catch (e) {
      return content;
  ...
  static Future<bool> resizeImage(String inputPath, String outputPath) async {
    final originalFile = File(inputPath);
    final imageBytes = await originalFile.readAsBytes();
    final img.Image? image = img.decodeImage(imageBytes);

    if (image == null) {
      Log.e('Can\'t read image!');
      return false;
    }

    final img.Image resizedImage = img.copyResize(image, width: 256);

    final resizedFile = File(outputPath);
    await resizedFile.writeAsBytes(img.encodeJpg(resizedImage));

    Log.d('Resize successful: $outputPath');
    return true;
  }

  static String formatPercent(double value) {
    final roundedValue = double.parse(value.toStringAsFixed(1));
    final formatted = roundedValue % 1 == 0 ? intl.NumberFormat('0').format(roundedValue) : intl.NumberFormat('0.0').format(roundedValue);
    return '$formatted%';
  }
