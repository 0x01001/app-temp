...
class AppProgressBar extends StatefulWidget {
  AppProgressBar(
      ...
      this.displayText,
      this.displayTextStyle = const TextStyle(color: Color(0xFF000000), fontSize: 10),
...
class _AppProgressBarState extends State<AppProgressBar> with SingleTickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) => AnimatedProgressBar(animation: _animation, widget: widget);
...
class AnimatedProgressBar extends AnimatedWidget {
  const AnimatedProgressBar({required Animation<double> animation, required this.widget, super.key}) : super(listenable: animation);

  final AppProgressBar widget;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.changeColorValue != null) {
      final _colorTween = ColorTween(begin: widget.progressColor, end: widget.changeProgressColor);
      progressColor = _colorTween.transform(transformValue(animation.value, widget.changeColorValue, widget.maxValue, 5))!;
    }

    final progressWidgets = <Widget>[];
    final Widget progressWidget = Container(
      decoration: BoxDecoration(color: widget.progressGradient != null ? null : progressColor, gradient: widget.progressGradient, borderRadius: widget._borderRadius, border: widget.border),
    );
    progressWidgets.add(progressWidget);

    if (widget.displayText != null) {
      final Widget textProgress = Container(
        alignment: widget.direction == Axis.horizontal ? const FractionalOffset(1, 0.5) : (widget.verticalDirection == VerticalDirection.up ? const FractionalOffset(0.5, 0.05) : const FractionalOffset(0.5, 0.95)),
        child: Text(
    ...
    return Directionality(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: widget.backgroundColor, borderRadius: widget._borderRadius, border: widget.border),
        child: Flex(
