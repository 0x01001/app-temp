...
enum AppActionSheetStyle { roundedCard, filled }
...
Future<dynamic> showFLBottomSheet<T>({required BuildContext context, required WidgetBuilder builder}) {
  return showModalBottomSheet(context: context, builder: builder, backgroundColor: Colors.transparent);
}
...
class AppActionSheet extends StatelessWidget {
  ...
  /// Default value is [AppActionSheetStyle.roundedCard], like iOS style.
  ...
  bool _isRound() {
    return style == AppActionSheetStyle.roundedCard;
  }
  ...
  Widget _buildMainContent(BuildContext context) {
    final BorderRadius radius = borderRadius ?? (_isRound() ? BorderRadius.circular(_kCornerRadius) : BorderRadius.zero);
    final Widget blurContent = _wrapWithBackground(
        backgroundColor: CupertinoDynamicColor.resolve(backgroundColor ?? _kBackgroundColor, context),
  ...
  Widget _buildCancelButton() {
    final double top = _isRound() ? _kCancelButtonPadding : 0;
    return Padding(
      padding: EdgeInsets.only(top: top),
      child: _AppActionSheetCancelButton(
        isRound: _isRound(),
        child: cancelButton,
      ),
    );
  ...
  Widget build(BuildContext context) {
    final EdgeInsets margin = _isRound() ? const EdgeInsets.symmetric(horizontal: _kEdgeHorizontalPadding, vertical: _kEdgeVerticalPadding) : const EdgeInsets.only(top: _kEdgeVerticalPadding);
...
class _AppActionSheetCancelButton extends StatefulWidget {
  const _AppActionSheetCancelButton({required this.child, this.isRound = true});

  final Widget child;
  final bool isRound;

  @override
  _AppActionSheetCancelButtonState createState() => _AppActionSheetCancelButtonState();
}

class _AppActionSheetCancelButtonState extends State<_AppActionSheetCancelButton> {
  ...
  void _onTapDown(TapDownDetails event) {
    // setState(() {
    //   _backgroundColor = CupertinoDynamicColor.resolve(_kCancelPressedColor, context);
    // });
  }

  void _onTapUp(TapUpDetails event) {
    // setState(() {
    //   _backgroundColor = CupertinoDynamicColor.resolve(_backgroundColor, context);
    // });
  }
  ...
  @override
  Widget build(BuildContext context) {
    final BorderRadius? borderRadius = widget.isRound ? BorderRadius.circular(_kCornerRadius) : null;
