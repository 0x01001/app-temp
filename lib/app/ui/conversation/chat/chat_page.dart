...
class ChatPage extends BasePage<ChatState, AutoDisposeStateNotifierProvider<ChatProvider, AppState<ChatState>>> {
  ...
  Widget render(BuildContext context, WidgetRef ref) {
    ...
    //     )..messageList = ref.read(provider.select((value) => value.data.messages)).map((e) => e.toMessage()).toList());
    ...
    //     chatController.chatUsers = [...members, currentUser].distinctBy((element) => element.id).toList();
    //   }
    // });
    ...
    // PreferredSizeWidget _buildAppBar({required bool isAdmin, required WidgetRef ref, String? title}) {
