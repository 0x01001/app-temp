...
class RefreshTokenInterceptor extends BaseInterceptor {
  RefreshTokenInterceptor(this.appPreferences, this.refreshTokenService, this._noneAuthAppServerApiClient);
  ...
  Future<void> _requestWithNewToken(RequestOptions options, ErrorInterceptorHandler handler, String newAccessToken) {
    ...
    return _noneAuthAppServerApiClient.fetch(options).then((response) => handler.resolve(response)).catchError((e) => handler.next(DioException(requestOptions: options, error: e)));
  }
}
