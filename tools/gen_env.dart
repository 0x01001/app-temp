...
void main() {
  ...
  flavorsList.forEach((x) {
    allDartDefinesByEnv[x.flavorEnum] = readDartDefineFromEnv(x.envPath, x.name);
  });

  GeneratorEnv(allDartDefinesByEnv: allDartDefinesByEnv).call();
}
...
class GeneratorEnv {
  ...
  void call() {
    try {
      ...
      print('''Error: The content of settings.json or launch.json is empty or the trailing comma is redundant or comments are invalid
...
class SettingsJsonWriter extends LaunchJsonWriter {
  ...
  Map<String, String> convertDartDefineToEnvVariable() {
    ...
    dartDefinesByEnv.forEach((key, value) {
      final Flavor matchingFlavor = flavorsList.firstWhere((element) => element.flavorEnum == key);
      map.addAll(value.map((key, value) => MapEntry('${matchingFlavor.prefix}_$key', value))..remove('${matchingFlavor.prefix}_$flavorKey'));
    });
    return map;
  }
}
...
class LaunchJsonWriter {
  ...
  Map<String, dynamic> updateConfig(Map<String, dynamic> config, Flavor flavor) {
